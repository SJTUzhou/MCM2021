...
for direction in direction_list:
    ...
    container_volumes = [int(element[:2]) for element in temp_dataframe["CNTR_TYPE"]]
    revenue = int(freight_rates.sum())
    volume = np.sum(container_volumes)
    freight_rate = freight_rates.mean()
    myDict = {"DIRECTION": direction, "REVENUE":revenue, "SALES_VOLUME":volume, "FREIGHT_RATE":freight_rate}
